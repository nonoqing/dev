...
then return the required framed sections with empty bodies exactly:
...
Return exactly these framed sections, in this order:

<<<RAW_MEMORY_BEGIN>>>
[raw_memory plain text]
<<<RAW_MEMORY_END>>>

<<<ROLLOUT_SUMMARY_BEGIN>>>
[rollout_summary plain text]
<<<ROLLOUT_SUMMARY_END>>>

<<<ROLLOUT_SLUG_BEGIN>>>
[rollout_slug plain text]
<<<ROLLOUT_SLUG_END>>>
...
- The markers must appear exactly as shown, each on its own line.
- Empty-field no-op must use empty bodies for all three sections.
- No prose outside the framed sections.
- `rollout_slug` must be empty or a single line.
...
- If a session has partially related tasks, prefer splitting into separate task blocks and
...
3. Return the framed `raw_memory`, `rollout_summary`, and `rollout_slug` sections exactly.
   No markdown wrapper, no prose outside the framed sections.
