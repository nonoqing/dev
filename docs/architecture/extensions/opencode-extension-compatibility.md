...
[`OC-E0` 至 `OC-E3`](../../specs/plans/opencode-extension-compatibility-plan.md) 为准。

### 当前运行时护栏（P0）

在匹配的 `OC-R` 阶段落地并完成验证之前，**当前 P0 adapter 仍是 managed-package / static-preview 路径**。

- 不要把遗留 managed-package 路径扩展成目标 OpenCode 运行时模型。
- 不要把设计目标当成已经可用的产品能力。

模块级细则见就近 `AGENTS.md`（如 `src/crates/adapters/opencode-adapter`、`src/apps/cli`）。
...
阶段交付和退出标准见[粗粒度计划](../../specs/plans/opencode-extension-compatibility-plan.md)。
